...
export const SupabaseProvider = ({children}: { children: ReactNode }) => {
    ...
    const deleteBoard = async (id: string) => {
        return await supabase.from(BOARDS_TABLE).delete().match({id});
    };
    ...
    const findUsers = async (search: string) => {
        ...
        const {data} = await supabase.rpc("search_users", {search: search});
        return data;
    };
    ...
    const getRealtimeCardSubscription = (
        ...
        // Return both the subscription and an unsubscribe function
        return {
            subscription,
